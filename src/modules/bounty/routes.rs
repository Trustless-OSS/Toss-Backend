...
async fn push_milestone(
    State(state): State<AppState>,
    user: AuthedUser,
    Json(body): Json<PushMilestoneBody>,
) -> Result<Json<PushMilestoneResponse>, AppError> {
    ...
    let issue = get_issue_by_repo_and_github_id(&state, repo.id, body.github_issue_id)
        .await?
        .ok_or_else(|| AppError::bad_request("Issue is not in a valid state to connect wallet"))?;
...
async fn retry_issue(
    ...
    if issue.status == "pending" {
        let contributor = contributor
            .ok_or_else(|| AppError::bad_request("Contributor has not connected a wallet yet"))?;
    ...
    if issue.status == "active" {
        let gh_state =
            fetch_github_issue_state(&state, &repo.full_name, issue.github_issue_number).await?;
